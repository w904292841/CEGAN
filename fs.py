...
def read_dpt(img_dpt_path):
    ...
    # dpt = cv2.imread(img_dpt_path,cv2.IMREAD_UNCHANGED)[:,:,-1]
...
class ImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.img_num):
            ...
            if self.flag_fcd:
                focus_dist += [self.focus_dist[i]*1e6]

        if self.flag_out_depth:
            mats_output = np.concatenate((mats_output, mat_dpt), axis=2)

        sample = {'input': mats_input, 'output': mats_output, 'focus_dist': focus_dist}
...
class ToTensor(object):
    def __call__(self, sample):
        ...
        return {'input': torch.from_numpy(mats_input),
                ...
                'focus_dist':torch.tensor(sample['focus_dist'])}
...
def load_data(DATA_PATH='./data/', DATA_SET='fs_', DATA_NUM=6, INP_IMG_NUM=5, FLAG_SHUFFLE=False, FLAG_IO_DATA={'INP_RGB': True,'INP_COC': False,'INP_AIF': False,'INP_DIST':True,'OUT_COC': False,'OUT_DEPTH': True,}, TRAIN_SPLIT=0.8,
              WORKERS_NUM=4, BATCH_SIZE=16, DATASET_SHUFFLE=True, DATA_RATIO_STRATEGY=0, FOCUS_DIST=[0.1,.15,.3,0.7,1.5], F_NUMBER=1., MAX_DPT=3.):
    ...
    img_dataset = ImageDataset(root_dir=data_dir, transform_fnc=transforms.Compose([ToTensor()]),
                               ...
                               flag_inputs=[FLAG_IO_DATA['INP_RGB'], FLAG_IO_DATA['INP_COC'],FLAG_IO_DATA['INP_AIF'], FLAG_IO_DATA['INP_DIST']],
